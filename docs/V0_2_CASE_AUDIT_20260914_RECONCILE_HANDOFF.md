...
## 8. 人工输入信息专项评价（Human Gate 输入复盘）

复盘对象：本会话 6 次人工输入实录（`human-gate.json`）。核心结论：**门把人当"单选题答题器"用得很高效，但对选项之外的人工表达（疑问、自愿事实、组合意图）是单向 deaf 的**——三类输入全部落进 unmatched 桶被丢弃。

### 8.1 输入全景

| 门 | 问题 | 用户原始输入 | 结果 |
|---|---|---|---|
| HG001 | 输出载体（SCOP-79259163） | "**1和3方案，这个文件怎么读取出来呢？**" | 未命中 → 47s 后重答 "3"（弃推荐，选 csv_and_notify） |
| HG001 | StarRocks 来源（FACT-2b52f4fa） | "**1，提供了仅读取权限账号**" | 未命中 → 重答 "1"（=推荐） |
| HG001 | 全史取数（TRAD-b99cb776） | "3" | ✅ 命中（=推荐） |
| HG002 | 实现载体（SCOP-87f46fd7） | "1" | ✅ 命中（=推荐，43 秒完成） |

最终 4 个决策全部为裸数字选项（零自定义决策），推荐接受率 3/4；未命中率 2/6（33%），反问回应率 0/1。

### 8.2 好的地方

1. **问题设计质量高**：每问带 `why_human`（仓库为何不能自决）、选项 impact 有量化依据（"每店 10 点"、"≤58.9 万样本证据"、"点数=月数×10"）、推荐有仓库先例背书（D003 推荐对齐 13 号文档 §3 既有规则）——用户答的是有依据的选择题，不是开放题。
2. **作答摩擦极低**：纯数字即可命中，RC3 别名空间（数字形态）真实生效；HG002 从提问到作答仅 43 秒。
3. **Override 顺畅**：D001 用户弃推荐改选 csv_and_notify，协议无阻拦、无强制确认——人类权威优先于推荐，语义正确。

### 8.3 不好的地方

- **H1（高）选项之外的人工表达全部被当噪声丢弃**：① 反问（"这个文件怎么读取出来呢？"）静默等待重答、全程无回应（与 P2/P4 同源）；② 自愿提供的事实（"**仅读取权限**账号"）未随 D002 落盘为会话事实，后续 R007 凭证讨论与设计均未用上；③ 组合意图（"1和3"）无反馈——选项 3 本就包含 1 的 CSV 输出，但包含关系不透明，用户以冗余组合表达困惑。
- **H2（中）单选协议未向用户明示**：题干未标注单选，选项间包含/互斥关系不标注，直接诱发 "1和3"；unmatched 反馈不回显"输入了什么、为何未命中"。
- **H3（中）FACT 问题止步于"来源"，没要到实物**：D002 选项 impact 自己写了"表口径/去重键/时区需自行核对"，但门没有顺势收 DDL/表名——用户手中已有账号，R002（口径无法证明）本可在门内预防，最终以最贵形式（终态交接）回收。
- **H4（低）评审期 4 个问题的人工交互一次未发生**：REQU 候选包质量高（选项+影响+推荐齐全）却因预算耗尽未开门（P1 连锁）；handoff.md 又漏渲染 R007 候选（P6）。

### 8.4 改进建议（分层）

1. **答案协议层**（收益最大）：① 自由文本含问句 → 进 QUESTION_ONLY 澄清应答（门 UI/编排器回应后再要求作答）；② 命中选项但带附注（"1，提供了仅读取权限账号"）→ 附注捕获为 session fact 随决策落盘；③ unmatched 反馈回显原始输入与未命中原因（多选冲突/歧义/保留字），选项存在包含关系时提示"3 已涵盖 1"。
2. **问题设计层**：FACT 类问题若答案是"用户提供实物"（凭证/DDL/表名），直接开粘贴字段在门内收齐外部事实；选项显式标注包含/互斥；题干明示单选；独立问题同批提问（对应 P1）。
3. **流程与呈现层**：handoff.md 渲染全部 NEED_HUMAN 候选含 deferred（P6）；HUMAN_HANDOFF 允许带答案 resume（P2）。
4. **遥测（V0.3 顺带）**：统计推荐接受率、未命中率与重答耗时、门内反问率与回应率——本会话单组样本即 33% unmatched、100% 反问无回应，值得量化追踪。
